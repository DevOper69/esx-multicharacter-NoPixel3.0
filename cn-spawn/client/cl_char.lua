...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    ESX.TriggerServerCallback("cn-spawn:fetchPlayerCharacters", function(data) 
        if data.err then
            return
        end
...
--[[
    ...
    Description: dealing with cn-spawn functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)    
    if not isReset then
        ...
        ESX.TriggerServerCallback("cn-spawn:loginPlayer", function(data) 
...
function Login.SelectedChar(data)
    ...
    ESX.TriggerServerCallback("cn-spawn:selectCharacter", function(returnData) 
...
RegisterNetEvent("cn-spawn:finishedClothing")
AddEventHandler("cn-spawn:finishedClothing", function(endType)
