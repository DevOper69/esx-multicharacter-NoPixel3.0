...
function Login.NUICallback(data)
	...
	if data.action == "currentHover" then
		...
		local coords = ESX:ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
	...
	if data.action == "deleteCharacter" then
		...
		ESX.TriggerServerCallback("cn-spawn:deleteCharacter", function(deleted) 
			Login.getCharacters(true)
		end, data.actionData)
	end
	...
	if data.action == "newCharacter" then
		...
		ESX.TriggerServerCallback("cn-spawn:createCharacter", function(created) 
			if not created then
...
AddEventHandler("esx:spawnInitialized", function()
	Login.nativeStart()
end)
