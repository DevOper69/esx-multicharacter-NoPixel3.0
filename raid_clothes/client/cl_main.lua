...
AddEventHandler('esx:playerLoaded', function(playerData)
    ...
    ESX.TriggerServerCallback("skins:getSteamId", function(result)
...
RegisterNUICallback('customskin', function(data, cb)
    if canUseCustomSkins() then
        print("Set Custom Skin to ",data)
        local valid_model = isInSkins(data)
        if valid_model then
            print("Dont Using Skin Set skin to ",data)
...
RegisterNUICallback('escape', function(data, cb)
    ...
    if shouldSave and currentPrice > 0 then
        ESX.TriggerServerCallback("clothing:purchase", function(result) 
            if not result then 
...
AddEventHandler("esx:initialSpawnModelLoaded", function()
...
local function listenForKeypress(zoneName, zoneData, isFree)
    listening = true
    Citizen.CreateThread(function()
        ESX.TriggerServerCallback("PriceWithTaxString", function(result) 
            priceWithTax = result
